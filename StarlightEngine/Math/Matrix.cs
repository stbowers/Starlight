...
namespace StarlightEngine.Math
{
    ...
    public class FMat4 : FMat
    {
        ...
        {
            FMat4 inverse = new FMat4();

            // calculate transposed matrix of minors
            ...
            float det = (matrix[0, 0] * inverse[0, 0]) - (matrix[0, 1] * inverse[1, 0]) + (matrix[0, 2] * inverse[2, 0]) - (matrix[0, 3] * inverse[3, 0]);

            // if determinant != 0, continue making transposed matrix of minors
            if (det == 0)
            {
                return null;
            }
            ...
            // multiply by one over determinant
