...
namespace FinalProject.Graphics.Vulkan
{
	public class VulkanPipeline : IPipeline
	{
		...
		public struct VulkanPipelineCreateInfo
		{
			...
			public VulkanShader vertexShader;
			public VulkanShader fragmentShader;
			...
			public override int GetHashCode()
			{
				...
				{
					int hash = 17;
					hash = hash * 23 + vertexShader.GetHashCode();
					hash = hash * 23 + fragmentShader.GetHashCode();

					hash = hash * 23 + topology.GetHashCode();
					hash = hash * 23 + primitiveRestartEnable.GetHashCode();

					hash = hash * 23 + frontFaceCCW.GetHashCode();

					hash = hash * 23 + depthTestEnable.GetHashCode();
					hash = hash * 23 + depthWriteEnable.GetHashCode();

					hash = hash * 23 + clearColorAttachment.GetHashCode();
					hash = hash * 23 + clearDepthAttachment.GetHashCode();
		...
		private VulkanShader m_vertexShader;
		private VulkanShader m_fragmentShader;

		public VulkanPipeline(VulkanPipelineCreateInfo createInfo)
		{
			m_apiInfo = createInfo.apiInfo;
			m_vertexShader = createInfo.vertexShader;
			m_fragmentShader = createInfo.fragmentShader;
			...
			vertShaderStageInfo.Module = createInfo.vertexShader.Module;
			vertShaderStageInfo.Name = createInfo.vertexShader.EntryPoint;
			...
			fragShaderStageInfo.Module = createInfo.fragmentShader.Module;
			fragShaderStageInfo.Name = createInfo.fragmentShader.EntryPoint;
			...
			vertexInputInfo.VertexBindingDescriptions = createInfo.vertexShader.GetInputBindingDescriptions();
			vertexInputInfo.VertexAttributeDescriptions = createInfo.vertexShader.GetInputAttributeDescriptions();
			...
			setLayouts.AddRange(createInfo.vertexShader.GetDescriptorSetLayouts());
			setLayouts.AddRange(createInfo.fragmentShader.GetDescriptorSetLayouts());
		...
		public VulkanShader GetVertexShader()
		{
			return m_vertexShader;
		}

		public VulkanShader GetFragmentShader()
		{
